...
impl FromStr for Round {
    ...
    fn from_str(round_string: &str) -> Result<Round, Self::Err> {
        ...
        let (word, guesses_str) = match round_vec[..] {
            ...
            [word, guesses_str] if word.len() == 5 && guesses_str.len() == 5
                 => (word, guesses_str),
            _ => return Err(RoundParseError::BadLen),
        };

        let guesses_result: Result<Vec<Guess>, Self::Err> = 
            word.chars().zip(guesses_str.chars())
        ...
        Ok(Round { guesses: guesses_result? })
    }
}

#[test]
fn test_round_from_str() {
    assert_eq!("terns=XYGXY".parse::<Round>(),
            Ok(Round { guesses: vec![
                Guess::Missing('t'),
                Guess::Present('e'),
                Guess::Correct('r'),
                Guess::Missing('n'),
...
fn test_round_badlen() {
    assert_eq!("tern=XXXXX".parse::<Round>(),
               Err(RoundParseError::BadLen));
    assert_eq!("terns=XXXX".parse::<Round>(),
               Err(RoundParseError::BadLen));
    assert_eq!("terns".parse::<Round>(),
               Err(RoundParseError::BadLen));
    assert_eq!("terns=XXXXX=foo".parse::<Round>(),
               Err(RoundParseError::BadLen));
}

#[test]
fn test_round_bad_guess_char() {
    assert_eq!("terns=XXXXA".parse::<Round>(),
               Err(RoundParseError::BadGuessChar));
}

#[test]
fn test_round_missing_word() {
    assert_eq!("=XXXXX".parse::<Round>(),
               Err(RoundParseError::MissingWord));
}
